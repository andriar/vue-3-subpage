...
export const useQiscusLiveChatStore = defineStore('create-qiscus-live-chat-ori', () => {
  const isChatDirty = ref(false);
  const errorPostWidgetConfig = ref<any>();

  // state for welcome dialog
  const welcomeDialogState = reactive<IWelcomeDialogFormState>({
    ...WIDGET_DEFAULTS.WELCOME_DIALOG,
  });
  const originalWelcomeDialogState = ref<IWelcomeDialogFormState>(
  ...
  // state for chat form
  const chatFormState = reactive<IChatState>({ ...WIDGET_DEFAULTS.CHAT });
  const originalChatFormState = ref<IChatState>(JSON.parse(JSON.stringify(WIDGET_DEFAULTS.CHAT)));
